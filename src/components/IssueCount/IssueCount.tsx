...
const IssueCount = () => {
    ...
    return (
        <div className="w-[20%] p-2 rounded-lg bg-[#13161A]">
            <p className={`${selectedIssue ? 'text-gray-500' : "text-white"}`}>
                <span onClick={() => setSelectedIssue(null)}>Count of Issues </span>
                {selectedIssue && (
                    <span>
                        / <span onClick={() => setSelectedIssueType(null)} className={`${selectedIssueType ? 'text-gray-500' : "text-white"}`}>{selectedIssue.title}</span>
                    </span>
                )}
                {selectedIssueType && (
                    <span>
                        / <span className="text-white">Issue #{selectedIssueType.id}</span>
                    </span>
                )}
            </p>
